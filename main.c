...
#pragma config(Sensor, dgtl3,  intakeButt,     sensorTouch)
#pragma config(Sensor, dgtl4,  launcherButt,   sensorTouch)
#pragma config(Sensor, dgtl5,  launcherCock,   sensorTouch)
...
task usercontrol(){
	...
	while (true)
	{
		if(bVEXNETActive){
			...
			//intakebutt ~~~ when ball is in intake
			//launcherbutt ~~~ when ball is in launcher
			//launchercock ~~~ when launcher is starting to move back


			if(vexRT[Btn6D] == 1){
				if(isLauncherButton == false && sensorValue[launcherCock] == 0 && sensorValue[intakeButt] == 1){
					motor[intake] = 80;
					wait1Msec(2000);
					motor[intake] = 0;

					if(sensorValue[launcherButt] == 1){
						motor[LL] = 100;
						motor[LR] = 100;
						wait1Msec(5000);
						motor[LL] = 0;
